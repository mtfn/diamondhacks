...
export default function Terminal({ challenge }: { challenge: Challenge }) {
  ...
  useEffect(() => {
    ...
    // fetch session once
    fetch(`http://127.0.0.1:5000/sessions/${user.sub}`)
      .then((resp) => resp.json())
                ...
                onKeyDown={(e) => {
                  if (e.key == "ArrowUp" && !e.shiftKey) {
                    ...
                    if (commandBackIdx < session.length) {
                      const command = session[session.length - 1 - commandBackIdx].cmd;
                    ...
                    if (commandBackIdx > 1) {
                      setCommandBackIdx(commandBackIdx - 1);
                      const command = session[session.length - commandBackIdx + 1].cmd;
                      (e.target as HTMLTextAreaElement).value = command || "";
                    } else {
                      setCommandBackIdx(0);
